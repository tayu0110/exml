...
fn xml_parser_print_file_context_internal(input: XmlParserInputPtr, buf: &mut String) {
...
pub fn parser_print_file_context(input: XmlParserInputPtr) {
    ...
    xml_parser_print_file_context_internal(input, &mut buf);
    generic_error!("{buf}");
}
...
pub unsafe fn report_error(
    ...
    GLOBAL_STATE.with_borrow_mut(|state| {
        ...
        if !ctxt.is_null() {
            xml_parser_print_file_context_internal(input, &mut output);
            if !cur.is_null() {
                ...
                xml_parser_print_file_context_internal(cur, &mut output);
            }
        }
