...
use super::{XmlNodeSet, XmlXPathCompExpr, XmlXPathObject, compile::XmlXPathStepOp};

fn xml_xpath_debug_dump_node<'a>(
...
fn xml_xpath_debug_dump_node_list<'a>(
    output: &mut (impl Write + 'a),
    cur: Option<XmlGenericNodePtr>,
    depth: i32,
) {
...
fn xml_xpath_debug_dump_node_set<'a>(
    output: &mut (impl Write + 'a),
...
fn xml_xpath_debug_dump_value_tree<'a>(
    output: &mut (impl Write + 'a),
    cur: Option<&XmlNodeSet>,
    depth: i32,
) {
...
#[cfg(feature = "libxml_xptr_locs")]
fn xml_xpath_debug_dump_location_set<'a>(
    output: &mut (impl Write + 'a),
...
pub fn xml_xpath_debug_dump_object<'a>(
    output: &mut (impl Write + 'a),
...
fn xml_xpath_debug_dump_step_op<'a>(
    output: &mut (impl Write + 'a),
    comp: &XmlXPathCompExpr,
    op: Option<&XmlXPathStepOp>,
    depth: i32,
) {
    let shift = "  ".repeat(depth.clamp(0, 25) as usize);
        ...
        XmlXPathOp::XPathOpValue => {
            ...
            if op.ch1 >= 0 {
                xml_xpath_debug_dump_step_op(
                    output,
                    comp,
                    Some(&comp.steps[op.ch1 as usize]),
                    depth + 1,
                );
            }
            if op.ch2 >= 0 {
                xml_xpath_debug_dump_step_op(
                    output,
                    comp,
                    Some(&comp.steps[op.ch2 as usize]),
                    depth + 1,
                );
            }
            return;
    ...
    // finish:
    if op.ch1 >= 0 {
        xml_xpath_debug_dump_step_op(output, comp, Some(&comp.steps[op.ch1 as usize]), depth + 1);
    }
    if op.ch2 >= 0 {
        xml_xpath_debug_dump_step_op(output, comp, Some(&comp.steps[op.ch2 as usize]), depth + 1);
    }
}

/// Dumps the tree of the compiled XPath expression.
#[doc(alias = "xmlXPathDebugDumpCompExpr")]
pub fn xml_xpath_debug_dump_comp_expr<'a>(
    output: &mut (impl Write + 'a),
    comp: &XmlXPathCompExpr,
    depth: i32,
) {
    ...
    if comp.stream.is_some() {
        ...
        xml_xpath_debug_dump_step_op(
            output,
            comp,
            Some(&comp.steps[comp.last as usize]),
            depth + 1,
        );
    }
}
