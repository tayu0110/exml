...
use crate::{
    ...
    xpath::{
        XML_XPATH_CHECKNS, XML_XPATH_NAN, XML_XPATH_NOVAR, XPATH_MAX_STEPS, XmlXPathCompExpr,
    ...
    unsafe {
        ...
        match (*op).op {
            ...
            XmlXPathOp::XpathOpUnion => {
                ...
                cur = xml_xpath_comp_op_eval_last(
                    ctxt,
                ...
                if (*(*ctxt).context).op_limit != 0
                    && ((*arg1)
                        .nodesetval
                        .as_deref()
                        .is_some_and(|n| xml_xpath_check_op_limit(ctxt, n.len() as _) < 0)
                        || (*arg2)
                            .nodesetval
                            .as_deref()
                            .is_some_and(|n| xml_xpath_check_op_limit(ctxt, n.len() as _) < 0))
            ...
            XmlXPathOp::XpathOpNode => {
                if (*op).ch1 != -1 {
                    total +=
                        xml_xpath_comp_op_eval(ctxt, &raw mut (*comp).steps[(*op).ch1 as usize]);
                }
                CHECK_ERROR0!(ctxt);
                if (*op).ch2 != -1 {
                    total +=
                        xml_xpath_comp_op_eval(ctxt, &raw mut (*comp).steps[(*op).ch2 as usize]);
                }
                CHECK_ERROR0!(ctxt);
                (*ctxt).value_push(xml_xpath_cache_new_node_set(
                    (*ctxt).context,
            ...
            XmlXPathOp::XpathOpValue => {
                (*ctxt).value_push(xml_xpath_cache_object_copy(
                    (*ctxt).context,
    ...
    unsafe {
        ...
        match (*op).op {
            ...
            XmlXPathOp::XpathOpEqual => {
                ...
                (*ctxt).value_push(xml_xpath_cache_new_boolean((*ctxt).context, equal != 0));
            }
            XmlXPathOp::XpathOpCmp => {
                total += xml_xpath_comp_op_eval(ctxt, &raw mut (*comp).steps[(*op).ch1 as usize]);
                CHECK_ERROR0!(ctxt);
            ...
            XmlXPathOp::XpathOpVariable => 'to_break: {
                ...
                if (*op).value5.is_null() {
                    ...
                    let uri: *const XmlChar =
                        xml_xpath_ns_lookup((*ctxt).context, (*op).value5 as _);
                    if uri.is_null() {
                        generic_error!(
            ...
            XmlXPathOp::XpathOpRangeto => 'to_break: {
                ...
                'rangeto_error: {
                    ...
                    (*ctxt).value_push(xml_xptr_wrap_location_set(newlocset));
                }
                // rangeto_error:
                xml_xpath_release_object((*ctxt).context, obj);
        ...
        loop {
            ...
            match (*op).op {
                ...
                XmlXPathOp::XpathOpCollect => {
                    ...
                    xml_xpath_comp_op_eval(
                        ctxt,
                        &raw mut (*(*ctxt).comp).steps[(*op).ch1 as usize],
                    );
                    if (*ctxt).error != XmlXPathError::XPathExpressionOK as i32 {
                        return -1;
                    }
