...
mod legacy {
    ...
    use crate::{
        libxml::globals::{xml_free, xml_generic_error_context, xml_malloc},
        xml_generic_error,
    };
     ...
     * Returns the link containing the data or NULL
     */
    unsafe extern "C" fn xml_list_link_reverse_search(
        l: XmlListPtr,
        data: *mut c_void,
    ) -> XmlLinkPtr {
        if l.is_null() {
     ...
     * Returns the value associated to @data or NULL in case of error
     */
    pub unsafe extern "C" fn xml_list_reverse_search(
        l: XmlListPtr,
        data: *mut c_void,
    ) -> *mut c_void {
        if l.is_null() {
        ...
        unsafe {
            ...
            for n_cur in 0..GEN_NB_XML_LIST_PTR {
                for n_old in 0..GEN_NB_CONST_XML_LIST_PTR {
                    ...
                    let ret_val = xml_list_copy(cur, old as XmlListPtr);
                    desret_int(ret_val);
                    des_xml_list_ptr(n_cur, cur, 0);
                    des_const_xml_list_ptr(n_old, old as XmlListPtr, 1);
                    xml_reset_last_error();
                    if mem_base != xml_mem_blocks() {
                        leaks += 1;
                        eprint!(
