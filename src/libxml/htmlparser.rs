...
use std::{
    ffi::{c_char, c_int, c_uchar, c_uint},
...
use crate::{
    ...
    libxml::{
        ...
        xml_io::{
            ...
            xmlParserInputBufferCreateFd, xmlParserInputBufferCreateIO,
            xmlParserInputBufferCreateMem, xmlParserInputBufferPush, XmlInputCloseCallback,
            XmlInputReadCallback, XmlParserInputBufferPtr,
        },
        xmlerror::{
            xmlParserValidityError, xmlParserValidityWarning, xmlResetError, XmlParserErrors,
        },
        xmlstring::{
...
macro_rules! SHRINK {
    ($ctxt:expr) => {
        ...
        {
            crate::private::parser::xmlParserShrink($ctxt)
        }
    };
}
...
macro_rules! GROW {
    ($ctxt:expr) => {
        ...
        {
            crate::private::parser::xmlParserGrow($ctxt);
        }
    };
}
...
unsafe extern "C" fn html_current_char(ctxt: XmlParserCtxtPtr, len: *mut c_int) -> c_int {
    ...
    if (*ctxt).charset != XmlCharEncoding::XmlCharEncodingUtf8 as i32 {
        ...
        if guess.is_null() {
            xml_switch_encoding(ctxt, XmlCharEncoding::XmlCharEncoding8859_1);
        } else {
...
pub(crate) unsafe extern "C" fn html_parse_char_ref(ctxt: HtmlParserCtxtPtr) -> c_int {
        ...
        && (NXT!(ctxt, 2) == b'x' || NXT!(ctxt, 2) == b'X')
    {
        ...
        while CUR!(ctxt) != b';' {
            ...
            } else if CUR!(ctxt) >= b'a' && CUR!(ctxt) <= b'f' {
                if val < 0x110000 {
...
unsafe extern "C" fn html_parse_html_attribute(
     ...
     */
    while CUR!(ctxt) != 0 && CUR!(ctxt) != stop {
        if stop == 0 && CUR!(ctxt) == b'>' {
            break;
        }
...
unsafe extern "C" fn html_parse_start_tag(ctxt: HtmlParserCtxtPtr) -> c_int {
     ...
     */
    SKIP_BLANKS!(ctxt);
    'failed: while CUR!(ctxt) != 0
        && CUR!(ctxt) != b'>'
        && (CUR!(ctxt) != b'/' || NXT!(ctxt, 1) != b'>')
        && !matches!((*ctxt).instate, XmlParserInputState::XmlParserEOF)
    {
        GROW!(ctxt);
...
unsafe extern "C" fn html_parse_system_literal(ctxt: HtmlParserCtxtPtr) -> *mut XmlChar {
    ...
    let quote: c_int = CUR!(ctxt) as _;
    NEXT!(ctxt);

    if CUR_PTR!(ctxt) < BASE_PTR!(ctxt) {
        return ret;
    }
    let start_position: size_t = CUR_PTR!(ctxt).offset_from(BASE_PTR!(ctxt)) as _;

    #[allow(clippy::while_immutable_condition)]
    while CUR!(ctxt) != 0 && CUR!(ctxt) as i32 != quote {
        /* TODO: Handle UTF-8 */
    ...
    if CUR!(ctxt) as i32 != quote {
        ...
        if err == 0 {
            ret = xml_strndup(BASE_PTR!(ctxt).add(start_position), len as _);
        }
        NEXT!(ctxt);
    }
...
unsafe extern "C" fn html_parse_content(ctxt: HtmlParserCtxtPtr) {
    ...
    loop {
         ...
         */
        else if CUR!(ctxt) == b'<' && IS_ASCII_LETTER!(NXT!(ctxt, 1)) {
 ...
 */
#[cfg(feature = "push")]
pub unsafe extern "C" fn html_create_push_parser_ctxt(
    sax: HtmlSaxhandlerPtr,
    user_data: *mut c_void,
    chunk: *const c_char,
    size: c_int,
    ...
    if matches!(enc, XmlCharEncoding::XmlCharEncodingUtf8) || !(*buf).encoder.is_null() {
        (*ctxt).charset = XmlCharEncoding::XmlCharEncodingUtf8 as i32;
    }
    if filename.is_null() {
 ...
 * Returns the resulting document tree
 */
pub unsafe extern "C" fn html_read_fd(
    fd: c_int,
    url: *const c_char,
    encoding: *const c_char,
    options: c_int,
 ...
 * Returns the resulting document tree
 */
pub unsafe extern "C" fn html_ctxt_read_fd(
    ctxt: XmlParserCtxtPtr,
    fd: c_int,
    url: *const c_char,
    encoding: *const c_char,
