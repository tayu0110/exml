...
mod legacy {
    ...
    use crate::{
        ...
        private::threads::{xml_cleanup_mutex, xml_init_mutex},
    };
     ...
     */
    pub(in crate::libxml::dict) unsafe extern "C" fn xml_dict_compute_fast_key(
        name: *const XmlChar,
        mut namelen: c_int,
        seed: c_int,
    ) -> c_ulong {
    ...
    pub(in crate::libxml::dict) unsafe extern "C" fn xml_dict_compute_big_key(
        data: *const XmlChar,
        namelen: c_int,
        seed: c_int,
    ) -> u32 {
     ...
     */
    pub(in crate::libxml::dict) unsafe extern "C" fn xml_dict_compute_fast_qkey(
     ...
     * Neither of the two strings must be NULL.
     */
    pub(in crate::libxml::dict) unsafe extern "C" fn xml_dict_compute_big_qkey(
        prefix: *const XmlChar,
        plen: c_int,
        name: *const XmlChar,
        len: c_int,
        ...
        */
        for i in 0..oldsize {
            ...
            if keep_keys != 0 {
                ...
                okey = xml_dict_compute_key!(
                    dict,
                    (*olddict.add(i)).name,
                    (*olddict.add(i)).len as i32
                ) as _;
            }
    ...
    pub unsafe extern "C" fn xml_dict_lookup(
        ...
        if !(*dict).subdict.is_null() {
            ...
            let skey = if ((*dict).size == MIN_DICT_SIZE
