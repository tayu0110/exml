...
pub struct XmlSchemaRedef {
    ...
    pub(crate) item: XmlSchemaBasicItemPtr, /* The redefining component. */
    pub(crate) reference: XmlSchemaBasicItemPtr, /* The referencing component. */
    target: XmlSchemaBasicItemPtr,          /* The to-be-redefined component. */
    pub(crate) ref_name: *const XmlChar,    /* The name of the to-be-redefined component. */
    pub(crate) ref_target_ns: *const XmlChar, /* The target namespace of the
...
unsafe fn xml_schema_validate_qname(
    ...
    unsafe {
        ...
        let ns_name = (*vctxt).lookup_namespace(
            (!prefix.is_null())
                .then(|| CStr::from_ptr(prefix as *const i8).to_string_lossy())
                .as_deref(),
        );
...
unsafe fn xml_schema_vexpand_qname(
    ...
    unsafe {
        ...
        *local_name = null_mut();
        ...
        if let Some(href) = (*vctxt).lookup_namespace(
            (!prefix.is_null())
                .then(|| CStr::from_ptr(prefix as *const i8).to_string_lossy())
                .as_deref(),
        ) {
