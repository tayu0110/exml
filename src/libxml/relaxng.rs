...
use std::{
    any::type_name,
    ffi::{c_char, c_int, c_short, c_uint, CStr},
...
use crate::{
    ...
    libxml::{
        ...
        tree::{
            xmlAddChild, xmlAddPrevSibling, xmlCopyDoc, xmlFreeNode, xmlIsBlankNode, xmlNewChild,
            xmlNewDocNode, xmlNewDocText, xmlNodeGetContent, xmlNodeListGetString,
            xmlNodeSetContent, xmlSearchNs, xmlSetProp, xmlUnsetProp, xml_add_next_sibling,
            xml_doc_get_root_element, xml_free_doc, xml_get_prop, xml_has_prop, xml_node_get_base,
            xml_split_qname2, xml_unlink_node, xml_validate_ncname, XmlAttrPtr, XmlDocPtr,
...
pub struct XmlRelaxNGDefine {
    ...
    depth: c_short,                  /* used for the cycle detection */
    dflags: c_short,                 /* define related flags */
...
pub struct XmlRelaxNGParserCtxt {
    ...
    flags: c_int,                        /* parser flags */
    nb_errors: c_int,                    /* number of errors at parse time */
    nb_warnings: c_int,                  /* number of warnings at parse time */
    define: *const XmlChar,              /* the current define scope */
    def: XmlRelaxNGDefinePtr,            /* the current define */

    nb_interleaves: c_int,
    ...
    def_nr: c_int,                     /* number of defines used */
    def_max: c_int,                    /* number of defines allocated */
    def_tab: *mut XmlRelaxNGDefinePtr, /* pointer to the allocated definitions */
    ...
    doc_nr: c_int,                       /* Depth of the parsing stack */
    doc_max: c_int,                      /* Max depth of the parsing stack */
    doc_tab: *mut XmlRelaxNGDocumentPtr, /* array of docs */

    /* the include stack */
    inc: XmlRelaxNGIncludePtr,          /* Current parsed include */
    inc_nr: c_int,                      /* Depth of the include parsing stack */
    inc_max: c_int,                     /* Max depth of the parsing stack */
    ...
    crng: c_int,    /* compact syntax and other flags */
    freedoc: c_int, /* need to free the document */
}
...
pub struct XmlRelaxNGValidState {
    ...
    nb_attrs: c_int,        /* the number of attributes */
    max_attrs: c_int,       /* the size of attrs */
    nb_attr_left: c_int,    /* the number of attributes left to validate */
...
pub struct XmlRelaxNGStates {
    nb_state: c_int,  /* the number of states */
    max_state: c_int, /* the size of the array */
...
pub struct XmlRelaxNGValidCtxt {
    ...
    flags: c_int,          /* validation flags */
    depth: c_int,          /* validation depth */
    idref: c_int,          /* requires idref checking */
    err_no: c_int,         /* the first error found */
    ...
    err_nr: c_int,                    /* Depth of the error stack */
    err_max: c_int,                   /* Max depth of the error stack */
    ...
    free_states_nr: c_int,
    free_states_max: c_int,
    ...
    elem_nr: c_int,                   /* the number of element validated */
    elem_max: c_int,                  /* the max depth of elements */
    elem_tab: *mut XmlRegExecCtxtPtr, /* the stack of regexp runtime */
    pstate: c_int,                    /* progressive state */
    pnode: XmlNodePtr,                /* the current node */
    pdef: XmlRelaxNGDefinePtr,        /* the non-streamable definition */
    perr: c_int,                      /* signal error in content model
                                       * outside the regexp */
}
...
pub struct XmlRelaxNGDocument {
    ...
    content: XmlRelaxNGDefinePtr, /* the definitions */
    schema: XmlRelaxNGPtr,        /* the schema */
    external_ref: c_int,          /* 1 if an external ref */
}
 ...
 */
type XmlRelaxNGTypeHave = unsafe extern "C" fn(data: *mut c_void, typ: *const XmlChar) -> c_int;
    ...
    'main: for i in 0..(*ctxt).err_nr {
        ...
        if k < MAX_ERROR {
            for j in 0..i {
                ...
                if (*err).err == (*dup).err
                    && (*err).node == (*dup).node
                    && xml_str_equal((*err).arg1, (*dup).arg1) != 0
                    && xml_str_equal((*err).arg2, (*dup).arg2) != 0
                {
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
unsafe extern "C" fn xml_relaxng_schema_type_have(
    _data: *mut c_void,
    typ: *const XmlChar,
) -> c_int {
    if typ.is_null() {
...
unsafe extern "C" fn xml_relaxng_schema_type_check(
    ...
    let ret: c_int = xml_schema_val_predef_type_node(typ, value, result as _, node);
    if ret == 2 {
...
unsafe extern "C" fn xml_relaxng_schema_facet_check(
    ...
    if xml_str_equal(facetname, c"minInclusive".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetMininclusive;
    } else if xml_str_equal(facetname, c"minExclusive".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetMinexclusive;
    } else if xml_str_equal(facetname, c"maxInclusive".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetMaxinclusive;
    } else if xml_str_equal(facetname, c"maxExclusive".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetMaxexclusive;
    } else if xml_str_equal(facetname, c"totalDigits".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetTotaldigits;
    } else if xml_str_equal(facetname, c"fractionDigits".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetFractiondigits;
    } else if xml_str_equal(facetname, c"pattern".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetPattern;
    } else if xml_str_equal(facetname, c"enumeration".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetEnumeration;
    } else if xml_str_equal(facetname, c"whiteSpace".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetWhitespace;
    } else if xml_str_equal(facetname, c"length".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetLength;
    } else if xml_str_equal(facetname, c"maxLength".as_ptr() as _) != 0 {
        (*facet).typ = XmlSchemaTypeType::XmlSchemaFacetMaxlength;
    } else if xml_str_equal(facetname, c"minLength".as_ptr() as _) != 0 {
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error.
 */
unsafe extern "C" fn xml_relaxng_default_type_have(
    _data: *mut c_void,
    typ: *const XmlChar,
) -> c_int {
    if typ.is_null() {
    ...
    if xml_str_equal(typ, c"string".as_ptr() as _) != 0 {
        return 1;
    }
    if xml_str_equal(typ, c"token".as_ptr() as _) != 0 {
        return 1;
    }
    0
}
...
unsafe extern "C" fn xml_relaxng_normalize(
    ...
    let len: c_int = tmp.offset_from(str) as _;
...
unsafe extern "C" fn xml_relaxng_default_type_compare(
    ...
    if xml_str_equal(typ, c"string".as_ptr() as _) != 0 {
        ret = xml_str_equal(value1, value2);
    } else if xml_str_equal(typ, c"token".as_ptr() as _) != 0 {
        if xml_str_equal(value1, value2) == 0 {
            /*
            ...
            if nval.is_null() || nvalue.is_null() {
                ret = -1;
            } else if xml_str_equal(nval, nvalue) != 0 {
                ret = 1;
            } else {
                ret = 0;
            }
...
pub unsafe extern "C" fn xml_relaxng_new_doc_parser_ctxt(
    ...
    let copy: XmlDocPtr = xmlCopyDoc(doc, 1);
    if copy.is_null() {
        return null_mut();
    }
...
unsafe extern "C" fn xml_relaxng_element_match(
    ...
    let mut ret: c_int = 0;
    let mut oldflags: c_int = 0;

    if !(*define).name.is_null() && xml_str_equal((*elem).name, (*define).name) == 0 {
        VALID_ERR3!(
            ctxt,
            XmlRelaxNGValidErr::XmlRelaxngErrElemname,
            (*define).name,
    ...
    if !(*define).ns.is_null() && *(*define).ns.add(0) != 0 {
        if (*elem).ns.is_null() {
            ...
            return 0;
        } else if xml_str_equal((*(*elem).ns).href.load(Ordering::Relaxed), (*define).ns) == 0 {
            VALID_ERR3!(
                ctxt,
                XmlRelaxNGValidErr::XmlRelaxngErrElemwrongns,
                (*elem).name,
...
unsafe extern "C" fn xml_relaxng_compute_interleaves(
    ...
    let mut ret: c_int;
    let mut res: c_int;
    let mut nbgroups: c_int = 0;
    let mut nbchild: c_int = 0;
 ...
 */
unsafe extern "C" fn xml_relaxng_is_blank(mut str: *mut XmlChar) -> c_int {
    if str.is_null() {
        return 1;
    }
    while *str != 0 {
    ...
    while !cur.is_null() {
            ...
            ) != 0
        {
            if xml_str_equal((*cur).name, c"name".as_ptr() as _) != 0 {
                if xml_str_equal((*node).name, c"element".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"attribute".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"ref".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"parentRef".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"param".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"define".as_ptr() as _) == 0
                {
                    xml_rng_perr(
                        ctxt,
                        node,
            ...
            } else if xml_str_equal((*cur).name, c"type".as_ptr() as _) != 0 {
                if xml_str_equal((*node).name, c"value".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"data".as_ptr() as _) == 0
                {
                    xml_rng_perr(
                        ctxt,
                        node,
            ...
            } else if xml_str_equal((*cur).name, c"href".as_ptr() as _) != 0 {
                if xml_str_equal((*node).name, c"externalRef".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"include".as_ptr() as _) == 0
                {
                    xml_rng_perr(
                        ctxt,
                        node,
            ...
            } else if xml_str_equal((*cur).name, c"combine".as_ptr() as _) != 0 {
                if xml_str_equal((*node).name, c"start".as_ptr() as _) == 0
                    && xml_str_equal((*node).name, c"define".as_ptr() as _) == 0
                {
                    xml_rng_perr(
                        ctxt,
            ...
            } else if xml_str_equal((*cur).name, c"datatypeLibrary".as_ptr() as _) != 0 {
                let uri: XmlURIPtr;

                let val: *mut XmlChar = xmlNodeListGetString((*node).doc, (*cur).children, 1);
                if !val.is_null() {
                    if *val.add(0) != 0 {
            ...
            } else if xml_str_equal((*cur).name, c"ns".as_ptr() as _) == 0 {
                xml_rng_perr(
                    ctxt,
                    node,
...
unsafe extern "C" fn xml_relaxng_load_external_ref(
    ...
    for i in 0..(*ctxt).doc_nr {
        if xml_str_equal((*(*(*ctxt).doc_tab.add(i as usize))).href, url) != 0 {
            xml_rng_perr(
                ctxt,
                null_mut(),
    ...
    if !ns.is_null() {
        ...
        if !root.is_null() && xml_has_prop(root, c"ns".as_ptr() as _).is_null() {
            xmlSetProp(root, c"ns".as_ptr() as _, ns);
        }
    }
    ...
    while !tmp.is_null() {
        ...
        if name.is_null() && IS_RELAXNG!(tmp, c"start".as_ptr() as _) {
            ...
            if (!inc.is_null() && !(*inc).doc.is_null() && !(*(*inc).doc).children.is_null())
                && xml_str_equal((*(*(*inc).doc).children).name, c"grammar".as_ptr() as _) != 0
            {
                // #ifdef DEBUG_INCLUDE
...
unsafe extern "C" fn xml_relaxng_load_include(
    ...
    for i in 0..(*ctxt).inc_nr {
        if xml_str_equal((*(*(*ctxt).inc_tab.add(i as usize))).href, url) != 0 {
            xml_rng_perr(
                ctxt,
                null_mut(),
    ...
    while !cur.is_null() {
        if IS_RELAXNG!(cur, c"start".as_ptr() as _) {
            let found: c_int = xml_relaxng_remove_redefine(ctxt, url, (*root).children, null_mut());
            if found == 0 {
                xml_rng_perr(
                    ctxt,
                    node,
            ...
            if name.is_null() {
                ...
                let found: c_int = xml_relaxng_remove_redefine(ctxt, url, (*root).children, name);
                if found == 0 {
                    xml_rng_perr(
                        ctxt,
                        node,
            ...
            if (*cur).typ == XmlElementType::XmlElementNode {
                    ...
                    ) == 0
                {
                    if !(*cur).parent.is_null()
                        ...
                        && (xml_str_equal((*(*cur).parent).name, c"name".as_ptr() as _) != 0
                            || xml_str_equal((*(*cur).parent).name, c"value".as_ptr() as _) != 0
                            || xml_str_equal((*(*cur).parent).name, c"param".as_ptr() as _) != 0)
                    {
                        xml_rng_perr(
                            ctxt,
                            cur,
                    ...
                    if xml_str_equal((*cur).name, c"externalRef".as_ptr() as _) != 0 {
                        ...
                        xml_free(url as _);
                        (*cur).psvi = docu as _;
                    } else if xml_str_equal((*cur).name, c"include".as_ptr() as _) != 0 {
                        let mut ns: *mut XmlChar;
                        let mut tmp: XmlNodePtr;
                        ...
                        xml_free(url as _);
                        (*cur).psvi = incl as _;
                    } else if xml_str_equal((*cur).name, c"element".as_ptr() as _) != 0
                        || xml_str_equal((*cur).name, c"attribute".as_ptr() as _) != 0
                    {
                        let ns: *mut XmlChar;
                        ...
                        if !name.is_null() {
                            if (*cur).children.is_null() {
                                text = xmlNewChild(cur, (*cur).ns, c"name".as_ptr() as _, name);
                            } else {
                                let node: XmlNodePtr = xmlNewDocNode(
                                    (*cur).doc,
                                    (*cur).ns,
                                    c"name".as_ptr() as _,
                                    null_mut(),
                            ...
                            xmlUnsetProp(cur, c"name".as_ptr() as _);
                            xml_free(name as _);
                            ns = xml_get_prop(cur, c"ns".as_ptr() as _);
                            if !ns.is_null() {
                                if !text.is_null() {
                                    xmlSetProp(text, c"ns".as_ptr() as _, ns);
                                    /* xmlUnsetProp(cur, c"ns".as_ptr() as _); */
                                }
                                xml_free(ns as _);
                            } else if xml_str_equal((*cur).name, c"attribute".as_ptr() as _) != 0 {
                                xmlSetProp(text, c"ns".as_ptr() as _, c"".as_ptr() as _);
                            }
                        }
                    } else if xml_str_equal((*cur).name, c"name".as_ptr() as _) != 0
                        || xml_str_equal((*cur).name, c"nsName".as_ptr() as _) != 0
                        || xml_str_equal((*cur).name, c"value".as_ptr() as _) != 0
                    {
                        /*
                         * Simplification 4.8. name attribute of element
                         * and attribute elements
                        ...
                        if xml_has_prop(cur, c"ns".as_ptr() as _).is_null() {
                            ...
                            if ns.is_null() {
                                xmlSetProp(cur, c"ns".as_ptr() as _, c"".as_ptr() as _);
                            } else {
                                xmlSetProp(cur, c"ns".as_ptr() as _, ns);
                                xml_free(ns as _);
                            }
                        }
                        if xml_str_equal((*cur).name, c"name".as_ptr() as _) != 0 {
                             ...
                             */
                            let name: *mut XmlChar = xmlNodeGetContent(cur);
                            if !name.is_null() {
                                local = xml_split_qname2(name, addr_of_mut!(prefix));
                                if !local.is_null() {
                                    let ns: XmlNsPtr = xmlSearchNs((*cur).doc, cur, prefix);
                                    if ns.is_null() {
                                        xml_rng_perr(
                                            ctxt,
                                            cur,
                         ...
                         * 4.16
                         */
                        if xml_str_equal((*cur).name, c"nsName".as_ptr() as _) != 0
                    ...
                    } else if xml_str_equal((*cur).name, c"except".as_ptr() as _) != 0
                        && cur != root
                    {
                        let oldflags: c_int = (*ctxt).flags;
                    ...
                    } else if xml_str_equal((*cur).name, c"anyName".as_ptr() as _) != 0 {
                        /*
                         * 4.16
                         */
                     ...
                     * into a div
                     */
                    if xml_str_equal((*cur).name, c"div".as_ptr() as _) != 0 {
                        ...
                        while !child.is_null() {
                            if !ns.is_null() && xml_has_prop(child, c"ns".as_ptr() as _).is_null() {
                                xmlSetProp(child, c"ns".as_ptr() as _, ns);
                            }
                            tmp = (*child).next;
             ...
             */
                ...
                if IS_BLANK_NODE!(cur) {
                    if !(*cur).parent.is_null()
                        && (*(*cur).parent).typ == XmlElementType::XmlElementNode
                    {
                        if xml_str_equal((*(*cur).parent).name, c"value".as_ptr() as _) == 0
                            && xml_str_equal((*(*cur).parent).name, c"param".as_ptr() as _) == 0
                        {
                            delete = cur;
                        }
...
unsafe extern "C" fn xml_relaxng_check_combine(
    ...
    let mut choice_or_interleave: c_int = -1;
    ...
    while !cur.is_null() {
        ...
        if !combine.is_null() {
            if xml_str_equal(combine, c"choice".as_ptr() as _) != 0 {
            ...
            } else if xml_str_equal(combine, c"interleave".as_ptr() as _) != 0 {
                if choice_or_interleave == -1 {
...
unsafe extern "C" fn xml_relaxng_parse_name_class(
    ...
    if IS_RELAXNG!(node, c"name".as_ptr() as _) {
        ...
        if (*ctxt).flags & XML_RELAXNG_IN_ATTRIBUTE != 0
            && !val.is_null()
            && xml_str_equal(val, c"http://www.w3.org/2000/xmlns".as_ptr() as _) != 0
        {
            xml_rng_perr(
                ctxt,
                node,
        ...
        if (*ctxt).flags & XML_RELAXNG_IN_ATTRIBUTE != 0
            && !val.is_null()
            && *val.add(0) == 0
            && xml_str_equal((*ret).name, c"xmlns".as_ptr() as _) != 0
        {
            xml_rng_perr(
                ctxt,
                node,
        ...
        if (*ctxt).flags & XML_RELAXNG_IN_ATTRIBUTE != 0
            && !(*ret).ns.is_null()
            && xml_str_equal((*ret).ns, c"http://www.w3.org/2000/xmlns".as_ptr() as _) != 0
        {
            xml_rng_perr(
                ctxt,
                node,
...
unsafe extern "C" fn xml_relaxng_parse_data(
    ...
    if lib.is_null() {
        ...
        if let Some(have) = (*lib).have {
            ...
            if tmp != 1 {
                ...
                && (xml_str_equal((*def).name, c"IDREF".as_ptr() as _) != 0
                    || xml_str_equal((*def).name, c"IDREFS".as_ptr() as _) != 0)
            {
                (*ctxt).idref = 1;
            }
        } else {
    ...
    while !content.is_null() {
        if xml_str_equal((*content).name, c"param".as_ptr() as _) == 0 {
            break;
        }
        if xml_str_equal(
            library,
        ...
        ) != 0
        {
            ...
            while !content.is_null() && xml_str_equal((*content).name, c"param".as_ptr() as _) != 0
            {
                content = (*content).next;
            }
        } else {
     ...
     */
    if !content.is_null() && xml_str_equal((*content).name, c"except".as_ptr() as _) != 0 {
        let mut child: XmlNodePtr;
        let mut tmp2: XmlRelaxNGDefinePtr;
        let mut last: XmlRelaxNGDefinePtr = null_mut();
...
unsafe extern "C" fn xml_relaxng_parse_attribute(
    ...
    let old_flags: c_int = (*ctxt).flags;
...
unsafe extern "C" fn xml_relaxng_process_external_ref(
    ...
    if !docu.is_null() {
        ...
        if (*docu).content.is_null() {
             ...
             */
            if new_ns == 1 {
                xmlUnsetProp(root, c"ns".as_ptr() as _);
            }
        }
...
unsafe extern "C" fn xml_relaxng_parse_include(
    ...
    if xml_str_equal((*root).name, c"grammar".as_ptr() as _) == 0 {
        xml_rng_perr(
            ctxt,
            node,
            XmlParserErrors::XmlRngpGrammarMissing as i32,
...
unsafe extern "C" fn xml_relaxng_combine_start(
    ...
    while !cur.is_null() {
        if (*cur).node.is_null()
            ...
            || xml_str_equal((*(*(*cur).node).parent).name, c"start".as_ptr() as _) == 0
        {
            combine = null_mut();
            xml_rng_perr(
                ctxt,
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error
 */
unsafe extern "C" fn xml_relaxng_is_nullable(define: XmlRelaxNGDefinePtr) -> c_int {
    let mut ret: c_int;
...
unsafe extern "C" fn xml_relaxng_check_choice_determinism(
    ...
    let mut ret: c_int;
    let mut cur: XmlRelaxNGDefinePtr;
    let mut is_indeterminist: c_int = 0;
    let mut triage: XmlHashTablePtr = null_mut();
    let mut is_triable: c_int = 1;
    ...
    let is_nullable: c_int = xml_relaxng_is_nullable(def);
 ...
 * Returns 1 if yes, 0 if no and -1 in case of error
 */
unsafe extern "C" fn xml_relaxng_is_compilable(def: XmlRelaxNGDefinePtr) -> c_int {
    let mut ret: c_int = -1;
...
pub unsafe extern "C" fn xml_relaxng_dump_tree(output: *mut FILE, schema: XmlRelaxNGPtr) {
    use crate::libxml::tree::xmlDocDump;
...
unsafe extern "C" fn xml_relaxng_add_states_uniq(
    ...
    if (*states).nb_state >= (*states).max_state {
        let size: c_int = (*states).max_state * 2;
 ...
 */
unsafe extern "C" fn xml_relaxng_next_value(ctxt: XmlRelaxNGValidCtxtPtr) -> c_int {
...
unsafe extern "C" fn xml_relaxng_validate_value(
    ...
    match (*define).typ {
        ...
        XmlRelaxNGType::Text => {}
        XmlRelaxNGType::Value => {
            if xml_str_equal(value, (*define).value) == 0 {
                if !(*define).name.is_null() {
                    let lib: XmlRelaxNGTypeLibraryPtr = (*define).data as _;
                    ...
                    if nval.is_null() || nvalue.is_null() || xml_str_equal(nval, nvalue) == 0 {
                        ret = -1;
                    }
                    if !nval.is_null() {
                        xml_free(nval as _);
...
unsafe extern "C" fn xml_relaxng_attribute_match(
    ...
    if !(*define).name.is_null() && xml_str_equal((*define).name, (*prop).name) == 0 {
        return 0;
    }
    if !(*define).ns.is_null() {
        if *(*define).ns.add(0) == 0 {
            ...
            || xml_str_equal((*define).ns, (*(*prop).ns).href.load(Ordering::Relaxed)) == 0
        {
            return 0;
        }
    }
...
unsafe extern "C" fn xml_relaxng_validate_attribute(
    ...
    if !(*define).name.is_null() {
        ...
        for i in 0..(*(*ctxt).state).nb_attrs {
            ...
            if !tmp.is_null()
                && xml_str_equal((*define).name, (*tmp).name) != 0
...
unsafe extern "C" fn xml_relaxng_validate_compiled_callback(
    ...
    let ret: c_int = xml_relaxng_validate_definition(ctxt, define);
    if ret != 0 {
        (*ctxt).perr = ret;
    }
}
 ...
 */
unsafe extern "C" fn xml_relaxng_best_state(ctxt: XmlRelaxNGValidCtxtPtr) -> c_int {
    let mut state: XmlRelaxNGValidStatePtr;

    let mut tmp: c_int;
    let mut best: c_int = -1;
    let mut value: c_int = 1000000;
...
unsafe extern "C" fn xml_relaxng_log_best_error(ctxt: XmlRelaxNGValidCtxtPtr) {
    ...
    let best: c_int = xml_relaxng_best_state(ctxt);
...
unsafe extern "C" fn xml_relaxng_copy_valid_state(
    ...
    let max_attrs: c_uint = (*ret).max_attrs as _;
...
unsafe extern "C" fn xml_relaxng_equal_valid_state(
    ...
    if (*state1).value != (*state2).value && xml_str_equal((*state1).value, (*state2).value) == 0 {
        return 0;
    }
                ...
                if !cur.is_null()
                        ...
                        && ((*(*define).parent).typ != XmlRelaxNGType::Def
                            || xml_str_equal((*(*define).parent).name,
                                            c"open-name-class".as_ptr() as _) == 0)
                {
                    VALID_ERR2!(
                        ctxt,
                        XmlRelaxNGValidErr::XmlRelaxngErrInterextra,
                ...
                if (*ctxt).err_nr != 0 {
                    while !(*ctxt).err.is_null()
                        && (((*(*ctxt).err).err == XmlRelaxNGValidErr::XmlRelaxngErrElemname
                            && xml_str_equal((*(*ctxt).err).arg2, (*node).name) != 0)
                            || ((*(*ctxt).err).err == XmlRelaxNGValidErr::XmlRelaxngErrElemextrans
                                && xml_str_equal((*(*ctxt).err).arg1, (*node).name) != 0)
...
pub unsafe extern "C" fn xml_relaxng_validate_doc(
    ...
    let ret: c_int = xml_relaxng_validate_document(ctxt, doc);
    /*
...
pub unsafe extern "C" fn xml_relaxng_validate_push_cdata(
    ...
    let ret: c_int = xml_reg_exec_push_string((*ctxt).elem, c"#text".as_ptr() as _, ctxt as _);
    if ret < 0 {
        VALID_ERR2!(
            ctxt,
            XmlRelaxNGValidErr::XmlRelaxngErrTextwrong,
