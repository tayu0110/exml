...
use super::{
    ...
    tree::{
        xmlAddChild, xmlFreeNs, xmlNewDoc, xmlNewDocNode, xmlNewNs, xmlSaveFormatFileTo,
        xmlSearchNsByHref, xmlSetNsProp, xmlSetProp, xml_doc_get_root_element, xml_free_doc,
        xml_get_ns_prop, xml_get_prop, xml_new_dtd, xml_node_get_base, XmlDocPtr, XmlDtdPtr,
 ...
 */
unsafe extern "C" fn xml_free_catalog_entry(payload: *mut c_void, _name: *const XmlChar) {
...
unsafe extern "C" fn xmlParseSGMLCatalog(
    ...
    while !cur.is_null() && *cur.add(0) != 0 {
        ...
        if *cur.add(0) == b'-' && *cur.add(1) == b'-' {
            ...
            SKIP_BLANKS!(cur);
            if xml_str_equal(name, c"SYSTEM".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataSystem;
            } else if xml_str_equal(name, c"PUBLIC".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataPublic;
            } else if xml_str_equal(name, c"DELEGATE".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataDelegate;
            } else if xml_str_equal(name, c"ENTITY".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataEntity;
            } else if xml_str_equal(name, c"DOCTYPE".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataDoctype;
            } else if xml_str_equal(name, c"LINKTYPE".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataLinktype;
            } else if xml_str_equal(name, c"NOTATION".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataNotation;
            } else if xml_str_equal(name, c"SGMLDECL".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataSgmldecl;
            } else if xml_str_equal(name, c"DOCUMENT".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataDocument;
            } else if xml_str_equal(name, c"CATALOG".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataCatalog;
            } else if xml_str_equal(name, c"BASE".as_ptr() as _) != 0 {
                typ = XmlCatalogEntryType::SgmlCataBase;
            } else if xml_str_equal(name, c"OVERRIDE".as_ptr() as _) != 0 {
                xml_free(name as _);
                cur = xml_parse_sgml_catalog_name(cur, addr_of_mut!(name));
                if name.is_null() {
                    /* error */
 ...
 */
pub unsafe extern "C" fn xml_convert_sgmlcatalog(mut catal: XmlCatalogPtr) -> c_int {
 ...
 * the application.
 */
static XML_CATALOG_XMLFILES: AtomicPtr<XmlHashTable> = AtomicPtr::new(null_mut());
...
unsafe extern "C" fn xml_parse_xml_catalog_node(
    ...
    if xml_str_equal((*cur).name, c"group".as_ptr() as _) != 0 {
        ...
        xml_free(prop as _);
    } else if xml_str_equal((*cur).name, c"public".as_ptr() as _) != 0 {
    ...
    } else if xml_str_equal((*cur).name, c"system".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"rewriteSystem".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"delegatePublic".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"delegateSystem".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"uri".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"rewriteURI".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"delegateURI".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
    ...
    } else if xml_str_equal((*cur).name, c"nextCatalog".as_ptr() as _) != 0 {
        entry = xml_parse_xml_catalog_one_node(
            cur,
...
unsafe extern "C" fn xml_parse_xml_catalog_file(
    ...
    if !cur.is_null()
        && xml_str_equal((*cur).name, c"catalog".as_ptr() as _) != 0
        && !(*cur).ns.is_null()
        && !(*(*cur).ns).href.load(Ordering::Relaxed).is_null()
        && xml_str_equal(
...
unsafe extern "C" fn xml_get_xml_catalog_entry_type(name: *const XmlChar) -> XmlCatalogEntryType {
    ...
    } else if xml_str_equal(name, c"rewriteSystem".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataRewriteSystem;
    } else if xml_str_equal(name, c"delegatePublic".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataDelegatePublic;
    } else if xml_str_equal(name, c"delegateSystem".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataDelegateSystem;
    } else if xml_str_equal(name, c"uri".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataUri;
    } else if xml_str_equal(name, c"rewriteURI".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataRewriteUri;
    } else if xml_str_equal(name, c"delegateURI".as_ptr() as _) != 0 {
        typ = XmlCatalogEntryType::XmlCataDelegateUri;
    } else if xml_str_equal(name, c"nextCatalog".as_ptr() as _) != 0 {
...
unsafe extern "C" fn xml_add_xmlcatalog(
    ...
    if !cur.is_null() {
        while !cur.is_null() {
            if !orig.is_null() && ((*cur).typ == typ) && xml_str_equal(orig, (*cur).name) != 0 {
                if XML_DEBUG_CATALOGS.load(Ordering::Relaxed) != 0 {
                    xml_generic_error!(
                        xmlGenericErrorContext(),
...
unsafe extern "C" fn xml_del_xml_catalog(
    ...
    cur = (*catal).children;
    while !cur.is_null() {
        if (!(*cur).name.is_null() && xml_str_equal(value, (*cur).name) != 0)
            || xml_str_equal(value, (*cur).value) != 0
        {
            if XML_DEBUG_CATALOGS.load(Ordering::Relaxed) != 0 {
                if !(*cur).name.is_null() {
                    xml_generic_error!(
...
unsafe extern "C" fn xml_catalog_xml_resolve(
    ...
    if !sys_id.is_null() {
        ...
        while !cur.is_null() {
            match (*cur).typ {
                XmlCatalogEntryType::XmlCataSystem => {
                    if xml_str_equal(sys_id, (*cur).name) != 0 {
                        if XML_DEBUG_CATALOGS.load(Ordering::Relaxed) != 0 {
                            xml_generic_error!(
                                xmlGenericErrorContext(),
        ...
        if have_delegate != 0 {
            ...
            'b: while !cur.is_null() {
                ...
                {
                    for i in 0..nb_list {
                        if xml_str_equal((*cur).url, delegates[i]) != 0 {
                            cur = (*cur).next;
                            continue 'b;
                        }
                    }
    ...
    if !pub_id.is_null() {
        ...
        while !cur.is_null() {
            match (*cur).typ {
                XmlCatalogEntryType::XmlCataPublic => {
                    if xml_str_equal(pub_id, (*cur).name) != 0 {
                        if XML_DEBUG_CATALOGS.load(Ordering::Relaxed) != 0 {
                            xml_generic_error!(
                                xmlGenericErrorContext(),
...
unsafe extern "C" fn xml_catalog_list_xml_resolve(
    ...
    ) == 0
    {
        ...
        } else if xml_str_equal(pub_id, urn_id) != 0 {
...
unsafe extern "C" fn xml_catalog_xml_resolve_uri(
    ...
    while !cur.is_null() {
        match (*cur).typ {
            XmlCatalogEntryType::XmlCataUri => {
                if xml_str_equal(uri, (*cur).name) != 0 {
                    if XML_DEBUG_CATALOGS.load(Ordering::Relaxed) != 0 {
                        xml_generic_error!(
                            xmlGenericErrorContext(),
                            c"Found URI match %s\n".as_ptr() as _,
 ...
 */
#[cfg(feature = "output")]
unsafe extern "C" fn xml_catalog_dump_entry(
    payload: *mut c_void,
    data: *mut c_void,
    _name: *const XmlChar,
) {
    ...
    while !cur.is_null() {
        if (*cur).group == cgroup {
            match (*cur).typ {
                ...
                XmlCatalogEntryType::XmlCataNextCatalog => {
                    node = xmlNewDocNode(doc, ns, c"nextCatalog".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"catalog".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataNone => {}
                XmlCatalogEntryType::XmlCataGroup => {
                    node = xmlNewDocNode(doc, ns, c"group".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"id".as_ptr() as _, (*cur).name);
                    if !(*cur).value.is_null() {
                        let xns: XmlNsPtr =
                            xmlSearchNsByHref(doc, node, XML_XML_NAMESPACE.as_ptr() as _);
                        if !xns.is_null() {
                            xmlSetNsProp(node, xns, c"base".as_ptr() as _, (*cur).value);
                        }
                    }
                    match (*cur).prefer {
                        XmlCatalogPrefer::None => {}
                        XmlCatalogPrefer::Public => {
                            xmlSetProp(node, c"prefer".as_ptr() as _, c"public".as_ptr() as _);
                        }
                        XmlCatalogPrefer::System => {
                            xmlSetProp(node, c"prefer".as_ptr() as _, c"system".as_ptr() as _);
                        }
                    }
                ...
                XmlCatalogEntryType::XmlCataPublic => {
                    node = xmlNewDocNode(doc, ns, c"public".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"publicId".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"uri".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataSystem => {
                    node = xmlNewDocNode(doc, ns, c"system".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"systemId".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"uri".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataRewriteSystem => {
                    node = xmlNewDocNode(doc, ns, c"rewriteSystem".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"systemIdStartString".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"rewritePrefix".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataDelegatePublic => {
                    node = xmlNewDocNode(doc, ns, c"delegatePublic".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"publicIdStartString".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"catalog".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataDelegateSystem => {
                    node = xmlNewDocNode(doc, ns, c"delegateSystem".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"systemIdStartString".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"catalog".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataUri => {
                    node = xmlNewDocNode(doc, ns, c"uri".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"name".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"uri".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataRewriteUri => {
                    node = xmlNewDocNode(doc, ns, c"rewriteURI".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"uriStartString".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"rewritePrefix".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
                XmlCatalogEntryType::XmlCataDelegateUri => {
                    node = xmlNewDocNode(doc, ns, c"delegateURI".as_ptr() as _, null_mut());
                    xmlSetProp(node, c"uriStartString".as_ptr() as _, (*cur).name);
                    xmlSetProp(node, c"catalog".as_ptr() as _, (*cur).value);
                    xmlAddChild(catalog, node);
                }
...
unsafe extern "C" fn xml_dump_xml_catalog(out: *mut FILE, catal: XmlCatalogEntryPtr) -> c_int {
     ...
     * Rebuild a catalog
     */
    let doc: XmlDocPtr = xmlNewDoc(null_mut());
    if doc.is_null() {
        return -1;
    }
    ...
    xmlAddChild(doc as _, dtd as _);

    let ns: XmlNsPtr = xmlNewNs(null_mut(), XML_CATALOGS_NAMESPACE.as_ptr() as _, null_mut());
    if ns.is_null() {
        xml_free_doc(doc);
        return -1;
    }
    let catalog: XmlNodePtr = xmlNewDocNode(doc, ns, c"catalog".as_ptr() as _, null_mut());
    if catalog.is_null() {
        xmlFreeNs(ns);
        xml_free_doc(doc);
        return -1;
    ...
    let ret: c_int = xmlSaveFormatFileTo(buf, doc, null_mut(), 1);
 ...
 * catalog file hash.
 */
unsafe extern "C" fn xml_free_catalog_hash_entry_list(payload: *mut c_void, _name: *const XmlChar) {
...
pub unsafe extern "C" fn xml_catalog_add(
    ...
    if default_catalog.is_null() && xml_str_equal(typ, c"catalog".as_ptr() as _) != 0 {
