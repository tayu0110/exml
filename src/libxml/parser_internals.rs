...
use crate::libxml::dict::xmlDictOwns;
...
use crate::libxml::globals::xmlGenericErrorContext;
...
use crate::libxml::tree::{
    xmlNewDoc, xml_create_int_subset, XmlAttributeDefault, XmlAttributeType, XmlDocProperties,
...
use super::dict::{xmlDictFree, xmlDictLookup, xmlDictReference, xmlDictSetLimit};
use super::encoding::{
...
use super::tree::{
    xmlAddChild, xmlAddChildList, xmlDocCopyNode, xmlFreeNode, xmlFreeNodeList, xmlNewDocNode,
    xmlSetTreeDoc, xml_buf_content, xml_buf_end, xml_buf_shrink, xml_buf_use, xml_free_doc,
    xml_split_qname3, XmlDocPtr, XmlElementContentPtr, XmlElementType, XmlEnumerationPtr,
...
pub(crate) unsafe extern "C" fn xml_parse_name(ctxt: XmlParserCtxtPtr) -> *const XmlChar {
        ...
        || *input == b':'
    {
        ...
        if *input > 0 && *input < 0x80 {
            ...
            ret = xmlDictLookup((*ctxt).dict, (*(*ctxt).input).cur, count as i32);
...
unsafe extern "C" fn xml_parse_balanced_chunk_memory_internal(
    ...
    if (*oldctxt).my_doc.is_null() {
        new_doc = xmlNewDoc(c"1.0".as_ptr() as _);
        if new_doc.is_null() {
            (*ctxt).sax = oldsax;
            (*ctxt).dict = null_mut();
            xml_free_parser_ctxt(ctxt);
    ...
    let new_root: XmlNodePtr = xmlNewDocNode(
        (*ctxt).my_doc,
        null_mut(),
    ...
    xmlAddChild((*ctxt).my_doc as _, new_root);
    ...
    if !(*ctxt).my_doc.is_null() {
        xmlFreeNode((*(*ctxt).my_doc).children);
                ...
                || (matches!((*list).typ, XmlElementType::XmlTextNode) && (*list).next.is_null())
            {
                ...
                while !list.is_null() {
                    ...
                    if (*list).doc != (*ent).doc.load(Ordering::Relaxed) as _ {
                        xmlSetTreeDoc(list, (*ent).doc.load(Ordering::Relaxed));
                    }
    ...
    if (*ctxt).replace_entities != 0 {
        ...
        if !(*ctxt).node.is_null() {
                ...
                || matches!((*ctxt).parse_mode, XmlParserMode::XmlParseReader)
            {
                ...
                while !cur.is_null() {
                    nw = xmlDocCopyNode(cur, (*ctxt).my_doc, 1);
                    if !nw.is_null() {
                ...
                while !cur.is_null() {
                    ...
                    if !nw.is_null() {
                        ...
                        xmlAddChild(ent as XmlNodePtr, nw);
                    }
                ...
                let nbktext: *const XmlChar =
                    xmlDictLookup((*ctxt).dict, c"nbktext".as_ptr() as _, -1);
                if matches!(
                ...
                xmlAddChildList((*ctxt).node, (*ent).children.load(Ordering::Relaxed));
            }
...
mod __parser_internal_for_legacy {
    ...
    use crate::{
        libxml::{
            entities::XmlEntityPtr, globals::xmlGenericErrorContext, parser::XmlParserCtxtPtr,
...
mod tests {
    use crate::{
        libxml::{parser::xml_pop_input, xmlerror::xmlResetLastError, xmlmemory::xml_mem_blocks},
        test_util::*,
    };
