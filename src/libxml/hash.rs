...
mod legacy {
    ...
    use crate::libxml::{
        dict::{
            __xml_random, xml_dict_free, xml_dict_lookup, xml_dict_owns, xml_dict_reference,
            XmlDictPtr,
        },
     ...
     * Returns the newly created object, or NULL if an error occurred.
     */
    pub unsafe extern "C" fn xml_hash_create_dict(
        size: c_int,
        dict: XmlDictPtr,
    ) -> XmlHashTablePtr {
     ...
     */
    pub unsafe extern "C" fn xml_hash_default_deallocator(
        entry: *mut c_void,
        _name: *const XmlChar,
    ) {
        xml_free(entry);
    ...
    pub unsafe extern "C" fn xml_hash_qlookup3(
        ...
        let key: c_ulong =
            xml_hash_compute_qkey(table, prefix, name, prefix2, name2, prefix3, name3);
        if (*(*table).table.add(key as usize)).valid == 0 {
            return null_mut();
        }
        entry = (*table).table.add(key as usize);
