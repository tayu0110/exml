...
use std::{
    ffi::{c_char, c_int, c_uchar, c_uint, c_ulong, CString},
    mem::size_of,
    os::raw::c_void,
 ...
 */
pub(crate) unsafe extern "C" fn xmlXPathReleaseObject(
...
pub unsafe extern "C" fn xml_xpath_err(ctxt: XmlXPathParserContextPtr, mut error: c_int) {
    ...
    xmlResetError(addr_of_mut!((*(*ctxt).context).last_error));
...
pub unsafe extern "C" fn xml_xpath_ns_lookup(
    ...
    if xml_str_equal(prefix, c"xml".as_ptr() as _) != 0 {
...
pub unsafe extern "C" fn xml_xpath_free_parser_context(ctxt: XmlXPathParserContextPtr) {
    if !(*ctxt).value_tab.is_null() {
        for i in 0..(*ctxt).value_nr {
            if !(*ctxt).context.is_null() {
                xmlXPathReleaseObject((*ctxt).context, *(*ctxt).value_tab.add(i as usize));
            } else {
...
unsafe extern "C" fn xml_xpath_is_axis_name(name: *const XmlChar) -> Option<XmlXPathAxisVal> {
    ...
    match *name.add(0) {
        b'a' => {
            if xml_str_equal(name, c"ancestor".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisAncestor);
            }
            if xml_str_equal(name, c"ancestor-or-self".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisAncestorOrSelf);
            }
            if xml_str_equal(name, c"attribute".as_ptr() as _) != 0 {
        ...
        b'd' => {
            if xml_str_equal(name, c"descendant".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisDescendant);
            }
            if xml_str_equal(name, c"descendant-or-self".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisDescendantOrSelf);
            }
        }
        b'f' => {
            if xml_str_equal(name, c"following".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisFollowing);
            }
            if xml_str_equal(name, c"following-sibling".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisFollowingSibling);
            }
        }
        b'n' => {
            if xml_str_equal(name, c"namespace".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisNamespace);
            }
        }
        b'p' => {
            if xml_str_equal(name, c"parent".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisParent);
            }
            if xml_str_equal(name, c"preceding".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisPreceding);
            }
            if xml_str_equal(name, c"preceding-sibling".as_ptr() as _) != 0 {
                ret = Some(XmlXPathAxisVal::AxisPrecedingSibling);
            }
        }
        b's' => {
            if xml_str_equal(name, c"self".as_ptr() as _) != 0 {
...
unsafe extern "C" fn xml_xpath_comp_node_test(
    ...
    *prefix = null_mut();
    ...
    if CUR!(ctxt) == b'(' {
         ...
         */
        if xml_str_equal(name, c"comment".as_ptr() as _) != 0 {
            *typ = XmlXPathTypeVal::NodeTypeComment;
        } else if xml_str_equal(name, c"node".as_ptr() as _) != 0 {
            *typ = XmlXPathTypeVal::NodeTypeNode;
        } else if xml_str_equal(name, c"processing-instruction".as_ptr() as _) != 0 {
            *typ = XmlXPathTypeVal::NodeTypePI;
        } else if xml_str_equal(name, c"text".as_ptr() as _) != 0 {
...
unsafe extern "C" fn xml_xpath_comp_function_call(ctxt: XmlXPathParserContextPtr) {
     ...
     */
    if prefix.is_null() && *name.add(0) == b'c' && xml_str_equal(name, c"count".as_ptr() as _) != 0
    {
        sort = 0;
    }
    (*(*ctxt).comp).last = -1;
...
unsafe extern "C" fn xmlXPathNodeCollectAndTest(
    ...
    match axis {
        ...
        XmlXPathAxisVal::AxisDescendant => {
            last = null_mut();
            next = Some(xmlXPathNextDescendant);
        }
        XmlXPathAxisVal::AxisDescendantOrSelf => {
            last = null_mut();
            ...
            match test {
                ...
                XmlXPathTestVal::NodeTestPI => {
                    if matches!((*cur).typ, XmlElementType::XmlPiNode)
                        && (name.is_null() || xml_str_equal(name, (*cur).name) != 0)
                    {
                        xp_test_hit!(has_axis_range, pos, max_pos, add_node, seq, cur, ctxt, out_seq, merge_and_clear, to_bool, break_on_first_hit, 'main);
                    }
                }
                ...
                XmlXPathTestVal::NodeTestName => 'to_break: {
                    ...
                    match (*cur).typ {
                        XmlElementType::XmlElementNode => {
                            if xml_str_equal(name, (*cur).name) != 0 {
                                if prefix.is_null() {
                                    if (*cur).ns.is_null() {
                                        xp_test_hit!(has_axis_range, pos, max_pos, add_node, seq, cur, ctxt, out_seq, merge_and_clear, to_bool, break_on_first_hit, 'main);
                                    }
                        ...
                        XmlElementType::XmlAttributeNode => {
                            ...
                            if xml_str_equal(name, (*attr).name) != 0 {
                                if prefix.is_null() {
                                    if (*attr).ns.is_null()
 ...
 */
unsafe extern "C" fn xmlXPathCompSwap(op: XmlXPathStepOpPtr) {
    /*
...
unsafe extern "C" fn xml_xpath_comp_op_eval_last(
    ...
    match (*op).op {
        ...
        XmlXPathOp::XpathOpNode => {
            if (*op).ch1 != -1 {
                total += xmlXPathCompOpEval(ctxt, (*comp).steps.add((*op).ch1 as usize));
            }
            CHECK_ERROR0!(ctxt);
            if (*op).ch2 != -1 {
                total += xmlXPathCompOpEval(ctxt, (*comp).steps.add((*op).ch2 as usize));
            }
            CHECK_ERROR0!(ctxt);
            value_push(
                ctxt,
        ...
        XmlXPathOp::XpathOpCollect => {
            if (*op).ch1 == -1 {
                ...
                total += xmlXPathCompOpEval(ctxt, (*comp).steps.add((*op).ch1 as usize));
                CHECK_ERROR0!(ctxt);
                total += xmlXPathNodeCollectAndTest(ctxt, op, null_mut(), last, 0);
            }
        }
        XmlXPathOp::XpathOpValue => {
            value_push(
    ...
    match (*op).op {
        ...
        XmlXPathOp::XpathOpCollect => {
            if (*op).ch1 == -1 {
                ...
                total = xmlXPathCompOpEval(ctxt, (*comp).steps.add((*op).ch1 as usize));
                CHECK_ERROR0!(ctxt);
                total += xmlXPathNodeCollectAndTest(ctxt, op, first, null_mut(), 0);
            }
        }
        XmlXPathOp::XpathOpValue => {
            value_push(
...
unsafe extern "C" fn xmlXPathCompOpEval(
    ...
    match (*op).op {
        ...
        XmlXPathOp::XpathOpCollect => 'to_break: {
            ...
            total += xmlXPathNodeCollectAndTest(ctxt, op, null_mut(), null_mut(), 0);
        }
        XmlXPathOp::XpathOpValue => {
            value_push(
                ctxt,
            ...
            'rangeto_error: {
                if (*(*ctxt).value).typ == XmlXPathObjectType::XpathLocationset {
                    ...
                    for i in 0..(*oldlocset).loc_nr {
                        ...
                        if (*op).ch2 != -1 {
                            total +=
                                xmlXPathCompOpEval(ctxt, (*comp).steps.add((*op).ch2 as usize));
                        }
                        if (*ctxt).error != XmlXPathError::XpathExpressionOk as i32 {
...
unsafe extern "C" fn xml_xpath_comp_op_eval_to_boolean(
    ...
    loop {
        ...
        match (*op).op {
            ...
            XmlXPathOp::XpathOpCollect => {
                ...
                xmlXPathCompOpEval(ctxt, (*(*ctxt).comp).steps.add((*op).ch1 as usize));
                if (*ctxt).error != XmlXPathError::XpathExpressionOk as i32 {
                    return -1;
                }

                xmlXPathNodeCollectAndTest(ctxt, op, null_mut(), null_mut(), 1);
                if (*ctxt).error != XmlXPathError::XpathExpressionOk as i32 {
                    return -1;
                }
    ...
    if !res_obj.is_null() {
        ...
        xmlXPathReleaseObject((*ctxt).context, res_obj);
        return res;
    }
...
pub(crate) unsafe extern "C" fn xml_xpath_run_eval(
    ...
    if !(*(*ctxt).comp).stream.is_null() {
        ...
        if to_bool != 0 {
            /*
            	...
            	*/
            res = xmlXPathRunStreamEval((*ctxt).context, (*(*ctxt).comp).stream, null_mut(), 1);
            if res != -1 {
                return res;
            }
        } else {
    ...
    if to_bool != 0 {
        ...
        xmlXPathCompOpEval(ctxt, (*comp).steps.add((*comp).last as usize));
    }
 ...
 */
pub unsafe extern "C" fn xmlXPathNodeSetDel(cur: XmlNodeSetPtr, val: XmlNodePtr) {
    if cur.is_null() {
        return;
    }
    if val.is_null() {
    ...
    for i in 0..(*ns).node_nr {
        if xml_xpath_node_val_hash(*(*ns).node_tab.add(i as usize)) == hash {
            ...
            if !str2.is_null() && xml_str_equal(str, str2) != 0 {
                ...
                return 1;
            } else if str2.is_null() && xml_str_equal(str, c"".as_ptr() as _) != 0 {
                if neq != 0 {
                    continue;
                }
                return 1;
    ...
    match (*arg1).typ {
        ...
        XmlXPathObjectType::XpathString => {
            match (*arg2).typ {
                ...
                XmlXPathObjectType::XpathString => {
                    ret = xml_str_equal((*arg1).stringval, (*arg2).stringval);
                }
                XmlXPathObjectType::XpathNumber => {
                    value_push(ctxt, arg1);
                    xml_xpath_number_function(ctxt, 1);
...
pub unsafe extern "C" fn xml_xpath_is_node_type(name: *const XmlChar) -> c_int {
    ...
    if xml_str_equal(name, c"node".as_ptr() as _) != 0 {
        return 1;
    }
    if xml_str_equal(name, c"text".as_ptr() as _) != 0 {
        return 1;
    }
    if xml_str_equal(name, c"comment".as_ptr() as _) != 0 {
        return 1;
    }
    if xml_str_equal(name, c"processing-instruction".as_ptr() as _) != 0 {
        return 1;
    }
    0
}
        ...
        unsafe {
            ...
            for n_ctxt in 0..GEN_NB_XML_XPATH_PARSER_CONTEXT_PTR {
                for n_cur in 0..GEN_NB_XML_NODE_PTR {
                    ...
                    let ret_val = xmlXPathNextDescendant(ctxt, cur);
                    desret_xml_node_ptr(ret_val);
                    des_xml_xpath_parser_context_ptr(n_ctxt, ctxt, 0);
                    des_xml_node_ptr(n_cur, cur, 1);
                    xmlResetLastError();
