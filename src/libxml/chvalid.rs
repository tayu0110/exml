 ...
 */
pub unsafe extern "C" fn xmlCharInRange(val: c_uint, rptr: *const XmlChRangeGroup) -> c_int {
    let mut low: c_int;
    let mut high: c_int;
    let mut mid: c_int;
 ...
 */
#[macro_export]
macro_rules! XmlIsBaseCharQ {
    ( $c:expr ) => {
        if $c < 0x100 {
            $crate::XmlIsBaseCharCh!($c) as i32
        } else {
            $crate::libxml::chvalid::xmlCharInRange(
                $c,
 ...
 */
#[macro_export]
macro_rules! XmlIsBlankQ {
    ( $c:expr ) => {
        if $c < 0x100 {
            $crate::XmlIsBlankCh!($c) as _
        } else {
            0
        }
    };
 ...
 */
#[macro_export]
macro_rules! xmlIsDigitQ {
	($( $c:tt )*) => {
		if $($c)* < 0x100 {
            $crate::xmlIsDigit_ch!($($c)*) as i32
        } else {
            $crate::libxml::chvalid::xmlCharInRange($($c)*, &$crate::libxml::chvalid::XML_IS_DIGIT_GROUP as _)
        }
	}
}
 ...
 */
#[macro_export]
macro_rules! xmlIsExtenderQ{
	($( $c:tt )*) => {
		if ($($c)* < 0x100) {
            $crate::xmlIsExtender_ch!($($c)*) as _
        } else {
            $crate::libxml::chvalid::xmlCharInRange($($c)*, &$crate::libxml::chvalid::XML_IS_EXTENDER_GROUP as _)
        }
	}
}
        ...
        unsafe {
            for n_val in 0..GEN_NB_UNSIGNED_INT {
                for n_rptr in 0..GEN_NB_CONST_XML_CH_RANGE_GROUP_PTR {
                    ...
                    let ret_val = xmlCharInRange(val, rptr);
                    desret_int(ret_val);
    ...
    fn test_xml_is_ideographic() {
        ...
        unsafe {
            for n_ch in 0..GEN_NB_UNSIGNED_INT {
                ...
                let ret_val = xmlIsIdeographic(ch);
                desret_int(ret_val);
                des_unsigned_int(n_ch, ch, 0);
                xmlResetLastError();
                if mem_base != xml_mem_blocks() {
