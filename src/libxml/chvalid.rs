...
fn xml_is_char_ch(c: u8) -> bool {
    (0x9..=0xa).contains(&c) || c == 0xd || 0x20 <= c
}

#[doc(alias = "xmlIsCharQ")]
pub(crate) fn xml_is_char(c: u32) -> bool {
    if c < 0x100 {
        xml_is_char_ch(c as u8)
    } else {
...
pub(crate) fn xml_is_combining(c: u32) -> bool {
    if c < 0x100 {
        ...
        xml_char_in_range(c, &XML_IS_COMBINING_GROUP)
    }
}
...
fn xml_is_digit_ch(c: u8) -> bool {
    (0x30..=0x39).contains(&c)
}

#[doc(alias = "xmlIsDigitQ")]
pub(crate) fn xml_is_digit(c: u32) -> bool {
    if c < 0x100 {
        xml_is_digit_ch(c as u8)
    } else {
        xml_char_in_range(c, &XML_IS_DIGIT_GROUP as _)
    }
}
...
fn xml_is_extender_ch(c: u8) -> bool {
    c == 0xb7
}

#[doc(alias = "xmlIsExtenderQ")]
pub(crate) fn xml_is_extender(c: u32) -> bool {
    if c < 0x100 {
        xml_is_extender_ch(c as u8)
    } else {
        xml_char_in_range(c, &XML_IS_EXTENDER_GROUP)
    }
}
...
pub(crate) fn xml_is_ideographic(c: u32) -> bool {
    if c < 0x100 {
        false
    } else {
...
fn xml_is_pubid_char_ch(c: u8) -> bool {
    XML_IS_PUBID_CHAR_TAB[c as usize]
}

#[doc = "xmlIsPubidCharQ"]
pub(crate) fn xml_is_pubid_char(c: u32) -> bool {
    c < 0x100 && xml_is_pubid_char_ch(c as u8)
}
