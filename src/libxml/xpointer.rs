...
use crate::{
    ...
    tree::{NodeCommon, XmlDoc, XmlElementType, XmlNode},
    xpath::{
        internals::{
    ...
    match (*loc).typ {
        ...
        XmlXPathObjectType::XPathRange => {
            if !(*loc).user2.is_null() {
                ...
                if node == (*(*ctxt).context).doc as *mut XmlNode {
...
unsafe fn xml_xptr_eval_xptr_part(ctxt: XmlXPathParserContextPtr, mut name: *mut XmlChar) {
    ...
    *cur = 0;
        ...
        || xml_str_equal(name, c"xpath1".as_ptr() as _)
    {
        ...
        (*(*ctxt).context).node = (*(*ctxt).context).doc as *mut XmlNode;
            ...
            if tmp != init {
                if (*tmp).typ == XmlXPathObjectType::XPathNodeset {
                    ...
                    if set.map_or(true, |s| {
                        s.len() != 1 || s.get(0) != (*ctx).doc as *mut XmlNode
                    }) {
                        stack += 1;
                    }
                } else {
