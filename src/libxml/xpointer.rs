...
unsafe fn xml_xptr_new_location_set_nodes(
    ...
    unsafe {
        ...
        let loc = if let Some(end) = end {
            xml_xptr_location_set_create(xml_xptr_new_range_nodes(start, end))
        } else {
            xml_xptr_location_set_create(xml_xptr_new_collapsed_range(start))
        };
        (*ret).user = (!loc.is_null()).then_some(XmlXPathObjectUserData::LocationSet(loc));
        ret
    }
}
...
pub(crate) unsafe fn xml_xptr_wrap_location_set(val: XmlLocationSetPtr) -> XmlXPathObjectPtr {
    ...
    unsafe {
        ...
        std::ptr::write(&mut *ret, XmlXPathObject::default());
        (*ret).typ = XmlXPathObjectType::XPathLocationset;
        (*ret).user = (!val.is_null()).then_some(XmlXPathObjectUserData::LocationSet(val));
        ret
    }
}
...
unsafe fn xml_xptr_range_function(ctxt: &mut XmlXPathParserContext, nargs: usize) {
    unsafe {
        ...
        if ctxt.value().is_none_or(|value| {
            ...
            XP_ERROR!(Some(&mut *ctxt), XmlXPathError::XPathInvalidType as i32);
        }
        ...
        if (*set).typ == XmlXPathObjectType::XPathNodeset {
            ...
            if tmp.is_null() {
                XP_ERROR!(Some(&mut *ctxt), XmlXPathError::XPathMemoryError as i32);
            }
            set = tmp;
        }
        let oldset: XmlLocationSetPtr = (*set)
            .user
            .as_ref()
...
unsafe fn xml_xptr_string_range_function(ctxt: &mut XmlXPathParserContext, nargs: usize) {
    unsafe {
        ...
        if !(2..=4).contains(&nargs) {
            XP_ERROR!(Some(&mut *ctxt), XmlXPathError::XPathInvalidArity as i32);
        }
        ...
        'goto_error: {
            ...
            if newset.is_null() {
                xml_xpath_err(Some(ctxt), XmlXPathError::XPathMemoryError as i32);
                // goto error;
                break 'goto_error;
            }
...
unsafe fn xml_xptr_start_point_function(ctxt: &mut XmlXPathParserContext, nargs: usize) {
    unsafe {
        ...
        let oldset: XmlLocationSetPtr = (*obj)
            .user
            .as_ref()
            .and_then(|user| user.as_location_set())
            .copied()
...
unsafe fn xml_xptr_eval_xptr_part(ctxt: XmlXPathParserContextPtr, mut name: *mut XmlChar) {
    unsafe {
        ...
        if name.is_null() {
            XP_ERROR!(Some(&mut *ctxt), XmlXPathError::XPathExprError as i32);
        }
...
pub unsafe fn xml_xptr_eval_range_predicate(ctxt: XmlXPathParserContextPtr) {
    unsafe {
        ...
        if (*ctxt).current_char() != Some('[') {
            XP_ERROR!(
                Some(&mut *ctxt),
                XmlXPathError::XPathInvalidPredicateError as i32
            );
        }
        (*ctxt).next_char();
        (*ctxt).skip_blanks();
