...
use super::{
    ...
    xml_io::{
        xmlAllocOutputBuffer, xmlOutputBufferClose, xmlOutputBufferCreateFilename,
        xmlOutputBufferFlush, xmlOutputBufferWriteString, XmlOutputBufferPtr,
    },
...
unsafe extern "C" fn xml_c14n_err_param(extra: *const c_char) {
    __xml_raise_error!(
        ...
        null_mut(),
        null_mut(),
        XmlErrorDomain::XmlFromC14N as i32,
...
pub unsafe extern "C" fn xml_c14n_doc_dump_memory(
    ...
    *doc_txt_ptr = null_mut();
     ...
     */
    let buf: XmlOutputBufferPtr = xmlAllocOutputBuffer(null_mut());
    if buf.is_null() {
...
pub unsafe extern "C" fn xml_c14n_doc_save(
     ...
     */
    let buf: XmlOutputBufferPtr = xmlOutputBufferCreateFilename(filename, null_mut(), compression);
    if buf.is_null() {
...
unsafe extern "C" fn xml_c14n_new_ctx(
    ...
    if !(*buf).encoder.is_null() {
        xml_c14n_err(
            ctx,
            doc as _,
            XmlParserErrors::XmlC14nRequiresUtf8 as i32,
...
unsafe extern "C" fn xml_c14n_err_relative_namespace(ns_uri: *const c_char) {
    __xml_raise_error!(
        ...
        null_mut(),
        XmlErrorDomain::XmlFromC14N as i32,
        XmlParserErrors::XmlC14nRelativeNamespace as i32,
        XmlErrorLevel::XmlErrError,
        null_mut(),
        0,
        null_mut(),
...
unsafe extern "C" fn xml_c14n_print_namespaces(ns: XmlNsPtr, ctx: XmlC14NCtxPtr) -> c_int {
    ...
    if !(*ns).prefix.load(Ordering::Relaxed).is_null() {
        ...
        xmlOutputBufferWriteString((*ctx).buf, (*ns).prefix.load(Ordering::Relaxed) as _);
...
unsafe extern "C" fn xml_c14n_print_attrs(data: *const c_void, user: *mut c_void) -> c_int {
    ...
    if !value.is_null() {
        ...
        xml_free(value as _);
        if !buffer.is_null() {
            xmlOutputBufferWriteString((*ctx).buf, buffer as _);
...
unsafe extern "C" fn xml_c14n_process_element_node(
    ...
    if visible != 0 {
        ...
        xmlOutputBufferWriteString((*ctx).buf, (*cur).name as _);
    }
...
unsafe extern "C" fn xml_c14n_err_invalid_node(node_type: *const c_char, extra: *const c_char) {
    __xml_raise_error!(
        ...
        null_mut(),
        XmlErrorDomain::XmlFromC14N as i32,
        XmlParserErrors::XmlC14nInvalidNode as i32,
        XmlErrorLevel::XmlErrError,
        null_mut(),
        0,
        extra,
...
unsafe extern "C" fn xml_c14n_err_unknown_node(node_type: c_int, extra: *const c_char) {
    __xml_raise_error!(
        ...
        null_mut(),
        XmlErrorDomain::XmlFromC14N as i32,
        XmlParserErrors::XmlC14nUnknowNode as i32,
        XmlErrorLevel::XmlErrError,
        null_mut(),
        0,
        extra,
        ...
        XmlElementType::XmlPiNode => {
            ...
            if visible != 0 {
                if matches!((*ctx).pos, XmlC14NPosition::XmlC14NAfterDocumentElement) {
                    xmlOutputBufferWriteString((*ctx).buf, c"\x0A<?".as_ptr() as _);
                } else {
                    xmlOutputBufferWriteString((*ctx).buf, c"<?".as_ptr() as _);
                }
...
pub unsafe extern "C" fn xml_c14n_execute(
    ...
    if ctx.is_null() {
        xml_c14n_err(
            null_mut(),
            doc as XmlNodePtr,
            XmlParserErrors::XmlC14nCreateCtxt as i32,
