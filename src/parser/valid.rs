...
use crate::{
    ...
    tree::{
        ...
        XmlAttributeType, XmlDocPtr, XmlDtdPtr, XmlElement, XmlElementContentOccur,
...
impl XmlParserCtxt {
    ...
    pub unsafe fn vpush_element(&mut self, doc: XmlDocPtr, elem: XmlNodePtr, qname: &str) -> i32 {
        unsafe {
            let mut ret: i32 = 1;
                    ...
                    match elem_decl.etype {
                        ...
                        XmlElementTypeVal::XmlElementTypeMixed => {
                                    ...
                                    == XmlElementContentType::XmlElementContentPCDATA
                            {
                                ...
                                let name = state.node.name().unwrap().into_owned();
                                let node = state.node.into();
                                ret = self.validate_check_mixed(elem_decl.content, qname);
                                if ret != 1 {
                                    xml_err_valid_node(
                                        self,
                                        Some(node),
        ...
        unsafe {
            ...
            match (*content).typ {
                ...
                XmlElementContentType::XmlElementContentElement => {
                    ...
                    let content_name =
                        CStr::from_ptr((*content).name as *const i8).to_string_lossy();
                    let fullname = build_qname(
                        &content_name,
                        (!(*content).prefix.is_null())
                            .then(|| {
                                CStr::from_ptr((*content).prefix as *const i8).to_string_lossy()
                            })
                            .as_deref(),
                    );
                ...
                XmlElementContentType::XmlElementContentSeq => {
                    ...
                    while {
                        self.build_a_content_model((*content).c1, name);
                        content = (*content).c2;
                    ...
                    self.build_a_content_model(content, name);
    ...
    pub unsafe fn build_content_model(&mut self, mut elem: XmlElementPtr) -> i32 {
        ...
        unsafe {
            ...
            self.build_a_content_model(elem.content, elem.name.as_deref().unwrap());
            self.vctxt
                .am
                .as_mut()
                .unwrap()
    ...
    #[cfg(feature = "libxml_valid")]
    pub unsafe fn validate_root(&mut self, doc: XmlDocPtr) -> i32 {
            ...
            if let Some(int_subset) = doc.int_subset.filter(|dtd| dtd.name.is_some()) {
                ...
                if int_subset.name() != root.name() {
                    ...
                    xml_err_valid_node(
                        ...
                        format!(
                            "root and DTD name do not match '{root_name}' and '{subset_name}'\n"
                        )
                        .as_str(),
                        Some(&root_name),
                        Some(&subset_name),
    ...
    unsafe fn validate_element_content(
        &mut self,
    ...
    unsafe fn validate_attribute_value2(
        &mut self,
        mut doc: XmlDocPtr,
        name: &str,
        typ: XmlAttributeType,
    ...
    #[cfg(feature = "libxml_valid")]
    pub unsafe fn validate_dtd_final(&mut self, doc: XmlDocPtr) -> i32 {
        unsafe {
            if doc.int_subset.is_none() && doc.ext_subset.is_none() {
                return 0;
            }
    ...
    unsafe fn validate_attribute_callback(&mut self, cur: XmlAttributePtr) {
        unsafe {
            let mut ret: i32;
    ...
    unsafe fn validate_notation_callback(&mut self, cur: XmlEntityPtr) {
        unsafe {
                            ...
                            if matches!(
                                    ...
                                    && xml_str_equal((*next).prefix, (*(*cur).c1).prefix)
                                {
                                    ...
                                    if (*(*cur).c1).prefix.is_null() {
                                        ...
                                        let prefix =
                                            CStr::from_ptr((*(*cur).c1).prefix as *const i8)
                                                .to_string_lossy();
                                        xml_err_valid_node(
                                        self,
                                        Some(elem.into()),
    ...
    pub unsafe fn validate_notation_use(&mut self, doc: XmlDocPtr, notation_name: &str) -> i32 {
        unsafe {
    ...
    #[cfg(feature = "libxml_regexp")]
    unsafe fn validate_check_mixed(&mut self, mut cont: XmlElementContentPtr, qname: &str) -> i32 {
        unsafe {
                ...
                while !cont.is_null() {
                    if matches!((*cont).typ, XmlElementContentType::XmlElementContentElement) {
                        if !(*cont).prefix.is_null()
                            && CStr::from_ptr((*cont).prefix as *const i8).to_string_lossy()
                                == prefix
