...
use crate::{
    ...
    libxml::{
        parser::{XmlParserInputState, XmlParserMode, XmlParserOption},
        parser_internals::xml_parse_balanced_chunk_memory_internal,
        sax2::xml_sax2_get_entity,
    },
    parser::{
        XmlParserCtxt, XmlParserCtxtPtr, parse_external_entity_private, xml_err_msg_str,
        xml_fatal_err, xml_fatal_err_msg, xml_fatal_err_msg_int, xml_fatal_err_msg_str,
        xml_is_char, xml_warning_msg,
    },
    tree::{
...
use super::{parse_name, parse_string_name, parser_entity_check};
...
pub(crate) unsafe fn parse_reference(ctxt: &mut XmlParserCtxt) {
    unsafe {
        ...
        if ctxt.nth_byte(1) == b'#' {
            let mut out = [0; 16];
            let hex = ctxt.nth_byte(2);
            let Some(value) = parse_char_ref(ctxt) else {
                return;
            };
        ...
        let Some(mut ent) = parse_entity_ref(ctxt) else {
            return;
        };
                    ...
                    != 0)
        {
            ...
            if parser_entity_check(ctxt, oldsizeentcopy) != 0 {
                xml_free_node_list(list);
                return;
            }
        ...
        if ent.children.is_none() {
            ...
            if was_checked != 0 {
                ...
                if parser_entity_check(ctxt, 0) != 0 {
                    return;
                }
            }
        ...
        if was_checked != 0 && parser_entity_check(ctxt, ent.expanded_size) != 0 {
            return;
        }
