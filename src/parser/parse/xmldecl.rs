...
pub(crate) unsafe fn parse_xmldecl(ctxt: &mut XmlParserCtxt) {
    unsafe {
        ...
        ctxt.input_mut().unwrap().standalone = -2;
        ...
        ctxt.skip_blanks();
        ctxt.input_mut().unwrap().standalone = parse_sddecl(ctxt);
        ctxt.skip_blanks();
        if ctxt.content_bytes().starts_with(b"?>") {
            ctxt.advance(2);
...
pub(crate) unsafe fn parse_text_decl(ctxt: &mut XmlParserCtxt) {
    unsafe {
        ...
        ctxt.input_mut().unwrap().version = version.or(Some(XML_DEFAULT_VERSION.to_owned()));
