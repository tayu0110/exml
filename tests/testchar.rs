...
use exml::{
    ...
    globals::{GenericErrorContext, set_structured_error},
...
static DOCUMENT1: Mutex<[i8; 100]> = Mutex::new(
    const {
        let mut buf = [0; 100];
...
static DOCUMENT2: Mutex<[i8; 100]> = Mutex::new(
    const {
        let mut buf = [0; 100];
...
unsafe fn test_document_ranges() -> c_int {
    unsafe {
        ...
        let mut document1 = DOCUMENT1.lock().unwrap();
        let mut document2 = DOCUMENT2.lock().unwrap();
            ...
            for j in 0..0xFF {
                ...
                *LAST_ERROR.write().unwrap() = XmlParserErrors::XmlErrOK;
                ...
                if i & 0x80 != 0 && i & 0x40 == 0 {
                    ...
                    if last_error != XmlParserErrors::XmlErrInvalidChar {
                        eprintln!(
                            "Failed to detect invalid char for Bytes 0x{i:02X} 0x{j:02X}: {c}"
                        );
                        return 1;
                    }
                } else if i & 0x80 != 0 && i & 0x1E == 0 {
                    ...
                    if last_error != XmlParserErrors::XmlErrInvalidChar {
                        eprintln!(
                            "Failed to detect invalid char for Bytes 0x{i:02X} 0x{j:02X} 0x00"
                        );
                        return 1;
                    }
                ...
                for &low in &lows {
                    ...
                    *LAST_ERROR.write().unwrap() = XmlParserErrors::XmlErrOK;
                        ...
                        || (value > 0xFFFD && value < 0x10000)
                    {
                        ...
                        if last_error != XmlParserErrors::XmlErrInvalidChar {
                            eprintln!(
                                "Failed to detect invalid char 0x{value:04X} for Bytes 0x{i:02X} 0x{j:02X} 0x{nk:02X}"
                            );
                            return 1;
                        }
                    } else {
                        ...
                        *LAST_ERROR.write().unwrap() = XmlParserErrors::XmlErrOK;
                        ...
                        if i & 0xF8 == 0xF8 {
                            ...
                            assert_eq!(
                                last_error,
                                XmlParserErrors::XmlErrInvalidChar,
                                "Failed to detect invalid char for Bytes 0x{i:02X} 0x{j:02X} 0x{nk:02X} 0x{nl:02X}"
                            );
                            ...
                            || value > 0x10FFFF
                        {
                            ...
                            assert_eq!(
                                last_error,
                                XmlParserErrors::XmlErrInvalidChar,
                                "Failed to detect invalid char 0x{value:04X} for Bytes 0x{i:02X} 0x{j:02X} 0x{nk:02X} 0x{nl:02X}"
                            );
...
unsafe fn test_char_ranges() -> c_int {
    unsafe {
        ...
        let Some(buf) = XmlParserInputBuffer::from_memory(data.to_vec(), XmlCharEncoding::None)
        else {
            xml_free_parser_ctxt(ctxt);
